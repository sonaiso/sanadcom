...
interface KPICardProps {
  ...
  /** When true, a positive trend is rendered green; when false, green means negative. */
  ...
  return (
    <div className={`relative bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow border-l-4 ${color}`}>
      ...
      <div className="flex items-end justify-between">
        ...
        {trend !== undefined && (
          <span className="text-xs text-gray-500">{thisMonthLabel}</span>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      <KPICard
        ...
        thisMonthLabel={labels.thisMonth}
      />
